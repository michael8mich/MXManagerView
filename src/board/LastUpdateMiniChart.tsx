...
// Read thresholds from env (VITE_LAST_UPDATE_CHART_THRESHOLDS=2,7)
const thresholds = (() => {
  const raw = (import.meta as any).env?.VITE_LAST_UPDATE_CHART_THRESHOLDS;
...
export function LastUpdateMiniChart({ t, problem }: { t: TFn; problem: Problem }) {
  ...
  const ageText = t('tooltip.lastUpdatedAgeValue', { age: formatAgeFromMs(t, ageMs) });

  return (
    <div
      className="flex items-center gap-1"
      title={`${t('tooltip.lastUpdatedAge')}: ${ageText}`}
      aria-label={`${t('tooltip.lastUpdatedAge')}: ${ageText}`}
    >
